...
VehicleAction = function(vehicleEntity, action)
    ...
    if action == "change_lock_state" then
        if dstCheck >= Config.RangeCheck then return ESX.ShowNotification("Too far away from the vehicle to control it.", "info" , 5000) end
...
SpawnVehicle = function(vehicleProps)
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
        ...
        ESX.ShowNotification('Please move the vehicle that is in the way', "error" , 5000)
		return
	end
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                ...
                ESX.ShowNotification('This vehicle is on the streets, you cant take out 2 of the same vehicles', "error" , 5000)
				return 
			end
		end
	end
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        ESX.ShowNotification('You spawned your vehicle', "success" , 5000)
	...
	if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("garage:validateVehicle", function(valid)
			if valid then
                ...
                ESX.ShowNotification('You parked your vehicle', "success" , 5000)
			else
			    TaskLeaveVehicle(PlayerPedId(), vehicle, 0)
...
WaitForModel = function(model)
    ...
    if not IsModelValid(model) then
        return ESX.ShowNotification("This model does not exist ingame.Please Contact Staff on discord.","info" , 5000)
    end
